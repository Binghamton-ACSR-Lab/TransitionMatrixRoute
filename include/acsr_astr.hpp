...
#ifndef TRANSITIONMATRIXROUTE_ACSR_ASTR_HPP
#define TRANSITIONMATRIXROUTE_ACSR_ASTR_HPP
#include <chrono>
...
namespace acsr {
    ...
    void AcsrAstar::init(int n_wire,IndexType init_index,IndexType target_index,const std::vector<int>& divided_vec) {
    ...
    std::vector<IndexType>
    AcsrAstar::getNextStepIndexVecWithinSteps(int n_wires, IndexType index, IndexType target_index, const std::vector<int> &divided_vec, int winthin_step) {
        ...
        if(size==1){
            ...
            std::copy_if(vec.begin(),vec.end(),std::back_inserter(v),[n_wires,target_index,winthin_step](IndexType i){
    ...
    void AcsrAstar::step() {
        ...
        auto explore_node = *std::min_element(open_set.begin(),open_set.end(),[&](const std::pair<IndexType,NodePtr> & n1, const std::pair<IndexType,NodePtr> & n2){
            return getHeuristic(_n_wire,n1.first,_target_index)<getHeuristic(_n_wire,n2.first,_target_index);
        });
        ///propagated states
        auto candidate_vec = getNextStepIndexVecWithinSteps(_n_wire,explore_node.first,_target_index,_divided_vec,best_solution.first-explore_node.second->getLevel()-1);
        ...
        for(auto index:candidate_vec){
            ...
            if(open_set.find(index)!=open_set.end()){
                ...
                if(previous_node->getLevel()>level || (previous_node->getLevel()==level && previous_node->getPathQuality()>quality)){
            ...
            if(target!= nullptr && (target->getLevel()!=best_solution.first || target->getPathQuality()!=best_solution.second)){
                best_solution.first = target->getLevel();
                best_solution.second = target->getPathQuality();
...
#endif //TRANSITIONMATRIXROUTE_ACSR_ASTR_HPP
