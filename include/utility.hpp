...
namespace acsr {
    ...
    void exportSVG(int n_wire,const std::vector<IndexType>& path,std::string file_name){
        ...
        for(auto i=0;i<4;++i){
            for(auto j=0;j<4;++j){
                acsr::Circle circle(acsr::Point(0.05*total_width + j*col_space,0.05*total_width + i*row_space),
        ...
        auto f = [&](std::pair<int,int> pt){
            return acsr::Point(0.05*total_width + pt.first*col_space,0.05*total_width + pt.second*col_space);
        };
